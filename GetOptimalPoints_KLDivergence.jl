...
function GetEvaluateValue_NextStep(x_measure::Vector{Int64}, x_reg::Vector{Int64}, sigma_measure::Vector{Float64}, sigma_reg::Vector{Float64}, kernel :: Matrix{Float64})::Vector{Float64}
    ...
    C_nxn = K_nxn + Diagonal(sigma_reg)
    C_nxn_inv = inv(C_nxn)
    ...
    for k :: Int64 = 1:size(x_reg)[1]
        ...
        C_mp1xmp1 = K_mp1xmp1 + Diagonal(sigma_measurep1)
        #C_mp1xmp1_inv = inv(C_mp1xmp1)

        #A = C_mp1xmp1_inv * transpose(K_nxmp1) * C_nxn_inv * K_nxmp1 * C_mp1xmp1_inv
        #S = Matrix{Float64}(I, size_n, size_n) - C_nxn_inv * K_nxmp1 * C_mp1xmp1_inv * transpose(K_nxmp1)

        #EvaluateValue_NextStep[k] = -logdet(S) + tr(S) + tr(A * Diagonal(sigma_measurep1) + A * K_mp1xmp1)
        EvaluateValue_NextStep[k] = -logdet(Matrix{Float64}(I, size_m+1, size_m+1) - inv(C_mp1xmp1) * K_mp1xmp1)
...
function OptimizeSampling(x_measure_init :: Vector{Int64}, x_reg :: Vector{Int64}, sigma_measure_init :: Vector{Float64}, sigma_reg::Vector{Float64}, SamplingNum::Int64, kernel :: Matrix{Float64})
    ...
    for k :: Int64 = tqdm(1:SamplingNum)
        ...
        argmin_eval = argmax(Evaluation_NextStep)
...
function do_Optimize(path_kernel, reg_energy, initial_sampling_num, SamplingNum, path_save_folder)
    ...
    reg_sigma = Spectrum2Noise(mean_spectrum, 23760, 1.75, 0.45)
...
SamplingNum = 320
path_save_folder = "results/OptimalPoints_KLDivergence"
